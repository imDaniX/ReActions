...
import me.fromgate.reactions.activators.storages.ItemHoldStorage;
...
import me.fromgate.reactions.activators.triggers.ItemHoldTrigger;
import me.fromgate.reactions.activators.triggers.ItemWearTrigger;
...
public class ItemStoragesManager {
    ...
    public void raiseItemWearActivator(Player player) {
        ...
        Bukkit.getScheduler().runTaskLater(ReActionsPlugin.getInstance(), () -> {
            for (Trigger iw : ActivatorsManager.getInstance().getActivators(ActivatorType.ITEM_WEAR))
                setFutureItemWearCheck(playerId, ((ItemWearTrigger) iw).getItemStr(), false);
        }, 1);
    }
    ...
    public void raiseItemHoldActivator(Player player) {
        final UUID playerId = player.getUniqueId();
        Bukkit.getScheduler().runTaskLater(ReActionsPlugin.getInstance(), () -> {
            for (Trigger ih : ActivatorsManager.getInstance().getActivators(ActivatorType.ITEM_HOLD))
                setFutureItemHoldCheck(playerId, ((ItemHoldTrigger) ih).getItemStr(), false);
        }, 1);
    }
