...
public class DamageTrigger extends Trigger {
    ...
    private DamageTrigger(ActivatorBase base, String cause, DamageType source) {
        super(base);
        this.damageCause = cause;
        this.source = source;
    }

    public static DamageTrigger create(ActivatorBase base, Parameters param) {
        String cause = param.getString("cause", "ANY");
        DamageType source = DamageType.getByName(param.getString("source", "ANY"));
        return new DamageTrigger(base, cause, source);
    }

    public static DamageTrigger load(ActivatorBase base, ConfigurationSection cfg) {
