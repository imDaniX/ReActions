...
public class ArgumentsChain implements Comparable {
	...
	public ArgumentsChain(String chain, ConfigurationSection argSection) {
		execs = new HashMap<>(); loadExecs(argSection);
	...
	public void tabComplete(List<String> complete, CommandSender sender, String[] args) {
		...
		if(sender instanceof ConsoleCommandSender) {
			...
			if(!Util.checkPermission(sender, permission)) return;
	...
	public ExecResult executeChain(CommandSender sender, String[] args) {
		...
		if(sender instanceof ConsoleCommandSender) {
			if(!consoleAllowed) return new ExecResult(ExecType.CONSOLE_DISALLOWED, getErroredExec(ExecType.CONSOLE_DISALLOWED));
		} else
			if(!Util.checkPermission(sender, permission)) return new ExecResult(ExecType.NO_PERMISSIONS, getErroredExec(ExecType.NO_PERMISSIONS));

		for(int i = 0; i < arguments.size(); i++) {
			ExecType resultType = arguments.get(i).check(args[i]);
	...
	private String getErroredExec(ExecType type) {
		return Util.searchNotNull(execs.get(ExecType.ANY_ERROR), execs.get(type));
	}
