...
public final class RaCommand extends Command implements PluginIdentifiableCommand {
	...
	public RaCommand(ConfigurationSection cmdSection, boolean register) {
		...
		execs = new HashMap<>(); loadExecs(cmdSection);
		chains = new TreeSet<>(); loadArguments(cmdSection);
	}
	...
	private void loadArguments(ConfigurationSection cmdSection) {
		...
		for(String arg : argsSection.getKeys(false))
			chains.add(new ArgumentsChain(arg, argsSection.getConfigurationSection(arg)));
	}
	...
	public final String executeCommand(CommandSender sender, String[] args) {
		if(sender instanceof ConsoleCommandSender) {
			...
			if(!Util.checkPermission(sender, permission)) return getErroredExec(ExecType.NO_PERMISSIONS);
		if(args.length == 0)
	...
	public final List<String> list() {
		List<String> list = Util.getEmptyList(1);
		chains.forEach(c -> list.add(c.toString()));
		return list;
	}

	private String getErroredExec(ExecType type) {
		return Util.searchNotNull("unknown", execs.get(type), execs.get(ExecType.ANY_ERROR), execs.get(ExecType.BACKUP), execs.get(ExecType.DEFAULT));
	}
}
